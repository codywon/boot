...
#include <compiler.h>
	...
	const int vfat_enabled = 1;
#else
	const int vfat_enabled = 0;
#endif
 ...
 */
int get_cluster(fsdata *mydata, __u32 clustnum, __u8 *buffer, unsigned long size)
{
	__u32 idx = 0;
	__u32 startsect;
	int ret;
...
__u8 mkcksum(const char name[8], const char ext[3])
{
	int i;
...
{
	//if(show_partition_error) printf("reading %s\n", filename);
	if(show_partition_error) printf("");
...
int fat_read_file(const char *filename, void *buf, int offset, int len)
{
	...
	if (len_read == -1) {
		//if(show_partition_error) printf("** Unable to read file %s **\n", filename);
		if(show_partition_error) printf("");
		return -1;
	}
