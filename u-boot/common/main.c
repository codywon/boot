...
static int cody_run(const char* command)
{
debug("Run \"%s\":\n",command);
...
static void usb_upgrade(void)
{
	...
	if(cody_run("usb reset"))
	{
		...
		if((filesize=load_file("cody/autorun",addr)) > 0)
		{
		...
		else // if autorun script was executed, no reflashing will be done
		{
			...
			filesize=load_file("cody/dump.bin",addr);
			...
			filesize=load_file("cody/u-boot.bin",addr);
			if(filesize > 0x10000)
			{
				partition=0;
				show_partition_error=was_show_error;
			...
			filesize=load_file("cody/u-boot-env.bin",addr);
			if(filesize == 0x10000)
			{
				partition=1;
				show_partition_error=was_show_error;
			...
			filesize=load_file("cody/firmware.bin",addr);
			if(filesize > 0)
			{
				partition=2;
				show_partition_error=was_show_error;
			...
			filesize=load_file("cody/art.bin",addr);
			if(filesize == 0x10000)
			{
				partition=3;
				show_partition_error=was_show_error;
			...
			if(load_file("cody/mac.bin",addr) == 6)
			{	
				show_partition_error=was_show_error;
...
#error "CONFIG_BOOTCOMMAND not defined!"
	...
	if(reset_button_status()){
		...
		printf("Press reset button for at least:\n"
			"- %d sec. to run upgrade from USB\n"
			"- %d sec. to run http update flash\n"
			"- %d sec. to reset system settings\n"
